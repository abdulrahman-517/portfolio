...
const PROJECTS = [
  ...
  { slug: 'q-search', title: 'Q Search', type: 'Search Tool', icon: 'search.svg', image: 'q-search.png', summary: 'An open search project with a public repository and live demonstration.', stack: ['Python', 'FastAPI'], overview: 'A public search-oriented project available for inspection through its repository and demonstration.', features: ['Search workflow', 'Public repository', 'Live demonstration'], capabilities: ['Python services', 'FastAPI', 'Deployment'], repo: 'https://github.com/abdulrahman-517/q-search', live: 'https://q-search-77si.onrender.com', approach: 'A compact project that keeps its public surface focused on the core search workflow.', limitations: 'The public demonstration represents the currently available scope.' }
];
...
function visual(project, detail = false) {
  return `<div class="project-visual${detail ? ' detail-visual' : ''}" data-project-image="${project.image}" aria-label="${esc(project.title)} project visual"><div class="project-placeholder">${icon(project.icon)}<span ${isArabic(project.title) ? 'dir="rtl"' : ''}>${esc(project.title)}</span></div></div>`;
}
...
function footer() {
  return `<footer class="site-footer"><div class="shell footer-inner"><a class="footer-brand" href="/"><img src="/assets/brand/am-lockup.svg" alt="Abdulrahman Al-Mushajari"></a><p>Building practical digital products with care and clarity.</p><div class="footer-links"><a href="${SITE.github}" target="_blank" rel="noreferrer" aria-label="GitHub">${icon('github.svg')}</a><a href="mailto:${SITE.email}" aria-label="Email">${icon('mail.svg')}</a></div></div></footer>`;
}
...
function projectPage(project) {
  ...
  const actions = [project.live && `<a class="button" href="${project.live}" target="_blank" rel="noreferrer">Open live project ${icon('external-link.svg')}</a>`, project.repo && `<a class="button button-quiet" href="${project.repo}" target="_blank" rel="noreferrer">${icon('github.svg')} View repository</a>`].filter(Boolean).join('');
